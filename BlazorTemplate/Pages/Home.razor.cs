...
public partial class Home(IDbContextFactory<ApplicationDbContext> dbFactory, ILogger<Home> logger)
{
    ...
    private async Task DeleteWorkerAsync(int workerId)
    {
        ...
        if (deletedCount != 0)
        {
            ...
            .Include(w => w.AssignedCompany)  // Include the assigned company
            .FirstOrDefaultAsync(w => w.WorkerId == workerId);

            if (worker?.AssignedCompany is not null)
            {
                var company = worker.AssignedCompany;

                // Step 2: Recalculate WorkerCount for the company
                company.WorkerCount = await db.Workers
                    .CountAsync(w => w.AssignedCompanyId == company.CompanyId);  // Count the remaining workers for the company

                // Step 3: Save the updated WorkerCount to the database
